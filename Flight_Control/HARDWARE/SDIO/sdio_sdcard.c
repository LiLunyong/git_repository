...
u32 total, free;
	...
	f_open(file, "0:/test.txt", FA_CREATE_ALWAYS | FA_WRITE | FA_READ); // 创建文件//文件指针+绝对路径+读写方式，全都要对
	strcpy(buf1, "Hello \r\n66.6.6  你干嘛 66 555.66\r\n");
	for (i = 0; i < 100; i++)
	{
		f_write(file, buf1, strlen(buf1), &br);
	}
	f_lseek(file, 0); // 指针返回文件开始处  不然f_read读不到数据
	f_read(file, fatbuf, strlen(buf1) * 100, &br);

	f_close(file);
	f_mount(NULL, "0:", 1);
	printf("lengthtext2:%d,buf2:%s\r\n", br, fatbuf);
